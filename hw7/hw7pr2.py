...
def find_cost(X, y, W , reg):
	...
	"*** YOUR CODE HERE ***"
	error = X@W - y
	cost = (float(error.T @ error) + reg*np.abs(W).sum())/len(y)
	"*** END YOUR CODE HERE ***"
	return cost
...
def grad_lasso(
	...
	while iter_num < max_iter and np.linalg.norm(W_grad) > eps:
		...
		"*** YOUR CODE HERE ***"
		indices = np.random.randint(size=batch_size, low=0, high=X.shape[0])
		gradient = find_grad(X[indices], y[indices], W, reg=reg)
		W = prox(W - lr * W_grad, reg * lr)
		cost = find_cost(X[indices], y[indices], W, reg)
