...
def find_RMSE(W, X, y):
	...
	"*** YOUR CODE HERE ***"
	y_error = y - predict(W, X)
	m = X.shape[0]
	RMSE = np.sqrt(np.linalg.norm(y_error, 2) ** 2 / m)
	"*** END YOUR CODE HERE ***"
	return RMSE
...
def RMSE_vs_lambda(X_train, y_train, X_val, y_val):
	...
	for i in range(len(reg_list)):
		W = linreg(X_train, y_train, reg_list[i])
		W_list.append(W)
	...
	"*** YOUR CODE HERE ***"
	smallest_RMSE = np.argmin(RMSE_list)
	reg_opt = reg_list[smallest_RMSE]
	"*** END YOUR CODE HERE ***"
	return reg_opt
...
def norm_vs_lambda(X_train, y_train, X_val, y_val):
	...
	for i in range(len(reg_list)):
		W = linreg(X_train, y_train, reg_list[i])
		W_list.append(W)
		norm_list.append(np.linalg.norm(W_list[i], 2))
	"*** END YOUR CODE HERE ***"
...
def linreg_no_bias(X, y, reg=0.0):
	...
	identity = np.eye(X.shape[1])	
	identity[0,0]=0
	w_help = X.T @ (np.eye(X.shape[0]) - np.ones(X.shape[0]) / X.shape[0])
	W_opt = np.linalg.solve(w_help @ X + reg * np.eye(w_help.shape[0]), w_help @ y)
	W_opt = np.linalg.solve(X.T@X + reg*identity, X.T@y)
