...
def NLL(X, y, W, reg=0.0):
	...
	"*** YOUR CODE HERE ***"
	pred = sigmoid(X @ W)
	temp = np.multiply(y, np.log(pred)) + np.multiply((1. - y), np.log(1. - pred))
	nll = (-sum(temp) + reg / 2 * np.linalg.norm(W) ** 2).item(0)
...
def get_description(X, y, W):
	...
	m,n=X.shape
	for i in range(m):
		pred_val = pred.item(i)
		actual_val = y.item(i)
		if pred_val==0 and actual_val==0:
			count_00+=1
		elif pred_val==0 and actual_val==1:
			count_01+=1
		elif pred_val==1 and actual_val==0:
			count_10+=1
		elif pred_val==1 and actual_val==1:
			count_11+=1
	accuracy = (count_00 + count_11)/(count_00 + count_11 + count_01 + count_10)
...
def plot_description(X_train, y_train, X_test, y_test):
	...
	"*** YOUR CODE HERE ***"
	reg_list = [0, 0.001, 0.01, 0.1, 0.5, 1, 5, 10, 30, 50]
	a_list = []
	p_list = []
	r_list = []
	f1_list = []
	for i in reg_list:
		W, obj = grad_descent(X_train, y_train, reg = i, lr = 2e-4, print_freq = 100)
	...
	"*** YOUR CODE HERE ***"
	optimal_index = np.argmax(a_list)
	reg_opt = reg_list[optimal_index]
