...
def k_means(X, k, eps=1e-10, max_iter=1000, print_freq=10):
	...
	while iter_num < max_iter:
		...
		for i in range(m):
			d = X[i,:]
			center = np.argsort(np.linalg.norm(d-clusters, axis=1)).item(0)
			label[i] = center

		for j in range(k):
			index = np.where(label == j)[0]
			if len(index) > 0:
				clusters[j,:] = X[index].mean(axis=0)
		"*** END YOUR CODE HERE ***"
...
def k_means_cost(X, clusters, label):
	...
	"*** YOUR CODE HERE ***"
	cluster = clusters[label.flatten()]
	cost = (np.linalg.norm(X - cluster, axis=1) ** 2).sum()
	"*** END YOUR CODE HERE ***"
	return cost
...
if __name__ == '__main__':
	...
	path = os.path.join(__location__, '5000_points.csv')
	columns = ['x', 'space', 'y']
	features = ['x', 'y']
	...
	cluster = clusters[label.flatten()]
	data_plot = plt.plot(X[:,0], X[:,1], 'ro')
	center_plot = plt.plot(cluster[:,0], cluster[:,1], 'bo')
	"*** END YOUR CODE HERE ***"
